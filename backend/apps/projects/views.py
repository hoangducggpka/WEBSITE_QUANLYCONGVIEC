...
# from apps.groups.models import Group
# from rest_framework.response import Response
# from .serializers import ProjectCreateSerializer, AddProjectMemberSerializer, ProjectListSerializer, ProjectDetailSerializer
# from .models import Project, UserProject
# from apps.tasks.models import Task
...
#     def delete(self, request, userproject_uuid):
#         try:
#             user_project = UserProject.objects.select_related(
#                 "project__group",
#                 "group_member__user"
#             ).get(uuid=userproject_uuid)
#         except UserProject.DoesNotExist:
#             return Response(
#                 {"error": "UserProject not found"},
...
#         group = project.group
#         leader = group.leader
#         kicked_user = user_project.group_member.user

#         if leader != request.user:
#             return Response(
#                 {"error": "Chỉ leader mới có thể kick thành viên!"},
#                 status=status.HTTP_403_FORBIDDEN
#             )
        
#         if kicked_user == leader:
#             return Response(
#                 {"error": "Bạn không thể tự kick bạn!"},
#                 status=status.HTTP_400_BAD_REQUEST
#             )

#         Task.objects.filter(
#             project=project,
#             assigned_to=user_project
#         ).delete()

#         leader_profile = UserProfile.objects.filter(user=leader).first()
#         leader_name = (
#             leader_profile.fullname
#             if leader_profile and leader_profile.fullname
#             else leader.username
#         )

#         content = f'{leader_name}|{group.name}| {leader_name} đã xóa bạn ra khỏi dự án "{project.name}"'
...
#         return Response(
#             {"message": "Đã xóa thành viên ra khỏi dự án!"},
...
# #         content = f'{leader_name}|{group_name}| {leader_name} đã xóa bạn ra khỏi dự án "{project_name}"'
...
#         if project.group.leader != request.user:
#             return Response(
#                 {"error": "Chỉ Leader mới có thể thay đổi thời gian dự án"},
#                 status=403
#             )
...
#             return Response(
#                 {"error": "Cần cung cấp ít nhất start_date hoặc end_date"},
#                 status=400
#             )

#         if start_date_str and project.computed_status == "ongoing":
#             return Response(
#                 {"error": "Không thể thay đổi start_date khi dự án đang diễn ra!"},
#                 status=400
#             )
...
#         if start_date_str:
#             start_date = self.parse_dt(start_date_str)
#             if not start_date:
#                 return Response(
#                     {"error": "Định dạng start_date không hợp lệ"},
#                     status=400
#                 )

#         if end_date_str:
#             end_date = self.parse_dt(end_date_str)
#             if not end_date:
#                 return Response(
#                     {"error": "Định dạng end_date không hợp lệ"},
#                     status=400
#                 )

#         if end_date_str and end_date <= project.end_date:
#             return Response(
#                 {"error": "Thời gian kết thúc mới phải lớn hơn thời gian kết thúc cũ!"},
#                 status=400
#             )
...
#             ).exists():
#                 return Response(
#                     {"error": "Có công việc bắt đầu trước thời gian bắt đầu mới của dự án!"},
#                     status=400
#                 )
...
#         leader_profile = getattr(request.user, "profile", None)
#         leader_name = (
#             leader_profile.fullname
#             if leader_profile and leader_profile.fullname
...
#         return Response(
#             {"message": "Thời gian dự án đã được update!"},
#             status=200
#         )
...
# #             return Response({"error": "start_date và end_date là bắt buộc"}, status=400)
...
# #         # Điều kiện 1: end_date mới phải > end_date cũ
# #         if end_date <= project.end_date:
# #             return Response({"error": "Thời gian kết thúc mới phải lớn hơn thời gian kết thúc cũ!"}, status=400)

# #         # Điều kiện 2: không cho start_date mới < bất kỳ task.start_date
# #         if Task.objects.filter(project=project, start_date__lt=start_date).exists():
# #             return Response({"error": "Có công việc bắt đầu trước thời gian bắt đầu mới của dự án!"}, status=400)
...
# #         content = f"{leader_name} |  | {leader_name} Đã thay đổi thời gian dự án '{project_name}'"
...
#         if group.leader != request.user:
#             return Response(
#                 {"error": "Chỉ trưởng nhóm với có thể tạo dự án!"},
#                 status=403
#             )
...
#     def post(self, request, project_uuid):
#         try:
#             project = Project.objects.select_related("group").get(#lọc và join
#                 uuid=project_uuid
#             )
#         except Project.DoesNotExist:
#             return Response({"error": "Project not found"}, status=404)
...
#         if project.group.leader != request.user:
#             return Response(
#                 {"error": "Only leader can add project members"},
#                 status=403
#             )
...
#         return Response({"message": "Member added to project"}, status=201)
...
#         notifications = []
#         for project in projects:
#             user_projects = UserProject.objects.filter(project=project).select_related(
#                 "group_member__user"
#             )
#             leader = project.group.leader
#             leader_fullname = leader.profile.fullname
            
#             for up in user_projects:
#                 notifications.append(Notification(
#                     user=up.group_member.user,
#                     project=None,
#                     content=f"{leader_fullname}||Dự án '{project.name}' đã bị xóa",
#                     priority=3
#                 ))
...
#             members__group_member__user=request.user
#         )

#         projects = (
#             leader_projects | member_projects
#         ).select_related(
#             "group",
#             "group__leader",
#             "group__leader__profile"
#         ).prefetch_related(
#             "members__group_member__user__profile"
#         ).distinct()
